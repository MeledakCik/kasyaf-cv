// lib/auth.ts - FINAL v5 FIX 3 ARGS + BACKWARD COMPAT
...
function timingSafeEqualHex(aHex: string, bHex: string): boolean {
  try {
    const a = fromHex(aHex); const b = fromHex(bHex);
...
export async function signSessionWithDevice(rawSessionId: string, deviceId: string, userAgent: string = ''): Promise<string> {
...
interface InternalTokenPayload { sid: string; path: string; exp: number; }
...
export async function signInternalToken(sessionId: string, pathname: string): Promise<string> {
  ...
  const encoded = base64urlEncode(JSON.stringify(payload));
  const sig = await hmacSha256Hex(INTERNAL_API_SECRET, encoded);
  return `${encoded}.${sig}`;
}

// === INI FIX ERROR "Expected 2 arguments, but got 3" ===
export async function verifyInternalToken(
  ...
  const expected = await hmacSha256Hex(INTERNAL_API_SECRET, encoded);
  ...
  try {
    ...
    if (pathnameOptional) {
      ...
      const expectedSid = (await hmacSha256Hex(INTERNAL_API_SECRET, sessionId)).slice(0, 16);
    ...
    // Fallback kalau dipanggil dengan sessionId doang (tanpa path)
    return payload;
  } catch { return null; }
}
