...
export async function solveChallenge(
  seed: string,
  difficulty: number
): Promise<{ nonce: string; solveTimeMs: number }> {
...
export function collectAutomationFlags(): string[] {
  ...
  // Navigator dengan properti webdriver opsional
  ...
  // Pengecekan window.chrome tanpa any
  if (typeof window !== "undefined") {
    const win = window as Window & { chrome?: unknown };
    if (typeof win.chrome === "undefined" && /Chrome/.test(navigator.userAgent)) {
