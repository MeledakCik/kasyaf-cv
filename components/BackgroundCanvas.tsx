...
const StarField = memo(({ isAbout }: { isAbout: boolean }) => {
  ...
  // Partikel dibuat sekali (stabil)
  const sphere = useMemo(() => {
    const data = new Float32Array(2000 * 3); // Turunkan dari 3000 -> 2000
    return random.inSphere(data, { radius: 1.8 }) as Float32Array;
  }, []);

  // Warna target berdasarkan isAbout
  ...
  useFrame((state, delta) => {
    ...
    // 3. 🟢 PERUBAHAN WARNA DI SINI (tanpa re-render React!)
    //    Lerp warna agar transisi halus, atau set langsung.
    ...
    <group rotation={[0, 0, Math.PI / 4]}>
      <Points ref={ref} positions={sphere} stride={3} frustumCulled={false}>
        <PointMaterial
          ...
          // Hapus props 'color' dari sini! Warnanya diatur via ref di useFrame
        />
      </Points>
    </group>
  );
